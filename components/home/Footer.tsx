    ...
    <footer className='w-full bg-white'>
      <div className='mx-auto flex min-h-[251px] max-w-pc flex-col items-center justify-between p-10 pb-5 lg:h-[180px] lg:flex-row lg:px-0 lg:pb-10'>
        <div className='flex flex-col items-center lg:items-stretch'>
          <h1 className='mb-4 text-xl font-bold text-black lg:h-8 lg:text-[32px]'>{t('title')}</h1>
        ...
        <div className='mt-5 flex flex-col items-center gap-y-5 lg:mt-0 lg:flex-row lg:items-stretch lg:gap-x-10'>
          <div className='flex w-full flex-col gap-2'>
            <h2 className='font-bold text-black'>{t('support')}</h2>
            {SupportLinks.map((item) => (
              <a
                ...
                target='_blank'
                rel='noreferrer'
                className='text-xs text-black hover:opacity-70 lg:text-base'
                title={item.title}
              >
                {item.title}
              </a>
          ...
          <div className='grid grid-cols-2 gap-x-10 gap-y-5 text-black lg:grid-cols-1 lg:gap-3'>
            {INFO_LIST.map((item) => (
              <InfoLink key={item.href} href={item.href} title={item.title} />
            ))}
            <a
              href={`mailto:${CONTACT_US_EMAIL}`}
              className='whitespace-nowrap text-xs text-black hover:opacity-70 lg:text-base'
              title={t('contactUs')}
              type='email'
            >
              {t('contactUs')}
